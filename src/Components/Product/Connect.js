    ...
    return {
        addItem: product => {
            ...
            fetch(`http://localhost:4000/users?id=${localStorage.user}`)
                ...
                .then(user => {
                    if (
                        user[0].cartItems.includes(
                            user[0].cartItems.filter(
                                item => item.id === product.id
                            )[0]
                        )
                    )
                        fetch(
                            `http://localhost:4000/users/${localStorage.user}`,
                            {
                                method: 'PATCH',
                                body: JSON.stringify({
                            ...
                            .then(res => res.json())
                            .then(user =>
                                dispatch(fetchCartItems(user.cartItems))
                            );
                    else
                        fetch(
                            `http://localhost:4000/users/${localStorage.user}`,
                            {
                                method: 'PATCH',
                                body: JSON.stringify({
                                    cartItems: user[0].cartItems.concat({
                                        ...product,
                                        quantity: 1
                                    })
                                }),
